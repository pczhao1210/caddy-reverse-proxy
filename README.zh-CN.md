...
- Console 可管理安全策略、登录令牌轮换、期望 Deployment 及 Azure DNS/NSG 设置。
- 支持安全的配置 ZIP 导入/导出，导入后先审阅再应用，且不包含证书或秘密材料。
...
脚本会在每次启动前拉取 `pczhao1210/caddy-reverse-proxy:latest`，并且只启动一个网关容器。80/443 对外发布，Console 仅绑定 `127.0.0.1:8080`，全部状态持久化到 `~/docker_files/caddy-reverse-proxy`。如果 `.env` 没有自定义管理员令牌，脚本会自动生成，在启动时显示一次，并随数据目录持久化。`start.sh` 要求 Docker 已可用；本机尚未安装 Docker 时，改用下方交互部署脚本的同机模式。

打开 `http://127.0.0.1:8080`，使用该令牌登录，然后在 Console 中配置路由、安全、设置和证书，并查看近期运行日志。路由编辑会保存为草稿，完成一批修改后点击**应用待处理更改**；安全策略与令牌变更会立即生效，但不会带上路由草稿。Deployment 和 Azure 集成会保存为重启后配置，因为它们影响启动期客户端和网络拓扑，启动器仍需提供对应 Docker 网络、Socket 和端口映射。申请 `*.example.com` 时，打开**证书**，同时添加 `*.example.com` 与 `example.com`，选择 Azure DNS 并填写 Azure 认证设置。通配符不覆盖根域名，因此根域名需要单独列出；已覆盖的一级具体域名会复用通配符，不再重复申请证书。

**设置 > 配置文件**可导出 `caddyproxy_config_yyyymmdd.zip`，也可导入同一固定格式的压缩包。ZIP 包含路由、Console 设置和证书申请策略，但绝不包含已签发证书、私钥、管理员令牌、Azure 客户端密钥、日志或 Caddy 运行数据。导入只会校验并载入内存草稿，不修改本地文件或 Caddy；审阅后点击**应用待处理更改**才会生效并持久化，Caddy 接受配置后会异步启动证书申请。导入的 Deployment 与 Azure 启动期设置会在应用时持久化，但需重启进程后才会创建对应的新客户端和拓扑。
...
选择第一种模式会创建 Azure 基础设施：交互选择订阅、区域、VNet、子网、VM 规格和系统盘，创建静态公网 IP、NSG、NIC、托管身份及 Ubuntu VM，安装 Docker 并启动网关。在当前机器部署时选择第二种模式；脚本会复用仓库中的 `start.sh`，或把最小 launcher 文件安装到 `~/caddy-reverse-proxy`，只启动容器，不修改 Azure 基础设施或 DNS。

已克隆仓库时可分别运行 `make azure-vm-deploy` 或 `make container-deploy`。部署镜像默认为 `pczhao1210/caddy-reverse-proxy:latest`，可通过 `IMAGE` 覆盖；Azure 模式支持 `ROLLBACK_ON_ERROR=false`，同机模式支持与 `start.sh` 相同的 `CONTAINER_NAME`、`DATA_DIR`、端口和 `DOCKER_NETWORKS` 覆盖。权限、持久化与网络约束见 [docs/deployment.zh-CN.md](docs/deployment.zh-CN.md)。
