...
ACI 可直接使用上方 Deploy to Azure 按钮。模板会创建 VNet、ACI、Standard Load Balancer、NAT Gateway、Azure Files 持久化和托管身份；镜像已预配置，因此只需填写管理员令牌。部署时提供 `dnsZones`，模板还会同时给控制面 UAMI 与 Caddy 的 system identity 授予 `DNS Zone Contributor`；其他证书设置可在 Console 启动后填写。
