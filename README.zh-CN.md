...
打开 `http://127.0.0.1:8080`，使用该令牌登录，然后在 Console 中配置路由、安全、设置和证书。安全策略与令牌变更会立即生效；Deployment 和 Azure 集成会保存为重启后配置，因为它们影响启动期客户端和网络拓扑，启动器仍需提供对应 Docker 网络、Socket 和端口映射。申请 `*.example.com` 时，打开 **证书**，同时添加 `*.example.com` 与 `example.com`，选择 Azure DNS 并填写 Azure 认证设置。通配符不覆盖根域名，因此根域名需要单独列出。
