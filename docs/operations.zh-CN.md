...
“证书”页面的后端接口为 `GET/PUT /api/certificate` 和 `POST /api/certificate/refresh`。变更会先原子保存到 `GATEWAY_CERTIFICATE_FILE`，再立即执行协调；Console 会分别报告持久化结果与 Caddy 重新加载失败。已保存设置会在重启后恢复。客户端密钥会持久化，但 API 永不回传。

Caddy 会在托管证书到期前自动续期。续期要求网关和 Caddy 运行时保持工作、`/data/caddy` 持久化，并且配置的 HTTP-01、TLS-ALPN-01 或 DNS-01 验证仍可使用。refresh 接口只通过协调重新加载当前 TLS 策略，不会强制续期。Console 当前还不显示单张证书的到期时间或续期尝试历史。
